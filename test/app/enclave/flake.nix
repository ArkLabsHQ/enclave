  ...
  outputs = { self, nixpkgs, flake-utils, aws-nitro-util }:
    flake-utils.lib.eachSystem [ "x86_64-linux" "aarch64-linux" "x86_64-darwin" "aarch64-darwin" ] (system:
      let
        ...
        # working-tree runtime instead of fetching a pinned release from
        # GitHub. Used by `make test-build` (see repo root Makefile). Not
        # part of the framework scaffolding — this file is test/app/flake.nix.
        ...
        # working tree instead of fetching a pinned commit from GitHub. Used
        # by `make test-build` so the test app builds against working-tree
        # code (the test app lives at <framework>/test/app/). Mirrors
        # RUNTIME_LOCAL_PATH above.
        ...
        upstream-app = eifPkgs.buildGoModule ({
          ...
          # Local-path builds can't rely on the yaml's vendor hash (it was
          # computed for a pinned commit, not the working tree). Set null so
          # Nix uses the local vendor/ dir — Makefile runs `go mod vendor`
          # in test/app before invoking this build.
