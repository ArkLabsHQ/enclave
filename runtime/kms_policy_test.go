...
func TestVerifyKeyPolicyPosture_Table(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "malformed JSON", policy: "{not json", expectedPCR0s: []string{ppPCR0}, locked: true,
			wantErr: true, errSubstr: "parse",
		},
		{
			name: "no statements", policy: ppPolicy(t), expectedPCR0s: []string{ppPCR0}, locked: true,
			wantErr: true, errSubstr: "PCR0 set",
		},
		{
