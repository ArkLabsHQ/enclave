...
test-build:  ## Build v1/v2/v3/v4 migration fixtures (G, A, and forked B/C successors)
	cd runtime && go mod vendor
	cd test/app && go mod vendor
	@set -eu; \
	cp test/app/enclave/enclave.yaml /tmp/enclave.yaml.original; \
	trap 'cp /tmp/enclave.yaml.original "$(CURDIR)/test/app/enclave/enclave.yaml"' EXIT; \
	...
	for version in 1 2 3 4; do \
		cp "/tmp/image-v$$version.eif" "test/app/.enclave/artifacts/image-v$$version.eif"; \
		cp "/tmp/pcr-v$$version.json" "test/app/.enclave/artifacts/pcr-v$$version.json"; \
	done
