...
let
  ...
  commonEifEnv = {
    ...
    # "inherited-from-outside". The second is already past its cutoff. The
    # third's cutoff is reached when e2e.py steps a node's clock to just
    # before it, so the restart-without-the-secret path runs for real.
