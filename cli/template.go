...
func runGenerateTemplate(outDir, language string) error {
	...
	// Write enclave.yaml with runtime hashes substituted.
	cfg := cfgTemplate
	if runtimeRev != "" {
...
Your app is a plain Go HTTP server. No runtime imports needed.
...
Your app is a plain Node.js HTTP server. No runtime imports needed.
...
Your app is a plain .NET HTTP server. No runtime imports needed.
...
# Runtime coordinates for the enclave supervisor binary.
runtime:
  rev: ""                        # runtime version (e.g. "v0.0.62")
  hash: ""                       # nix-prefetch-url --unpack (SRI hash)
  vendor_hash: ""                # Go vendor hash for runtime
...
2. Set up the runtime and compute hashes:
   enclave setup
