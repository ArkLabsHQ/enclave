...
| `<prefix>/<deployment>/enclave/metrics` | The periodic embedded-metric-format documents, covering both. |
...
Past the budget new names are refused while names already stored keep updating,
and every refusal is counted in `enclave_app_metrics_dropped_total`. The bound is
what keeps enclave memory and the number of metric documents shipped per interval
finite: the names, and the delta baselines kept for them, are the only telemetry
state that grows with what the application sends. OTLP log and span uploads are
separately capped at 10000 records per request, answered with `413` above that.
