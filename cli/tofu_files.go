...
data "aws_iam_policy_document" "enclave" {
  ...
  # SSM: mutable runtime state requires read/write access.
  statement {
...
# Also read the EIF and supervisor through data.local_file so their content_md5
# upload triggers are computed at apply time AFTER any download — otherwise a
# moving release_tag causes one-apply-late drift
...
locals {
  ...
  candidate_eif_key        = "candidates/${local.effective_pcr0}/enclave.eif"
  candidate_supervisor_key = "candidates/${local.effective_pcr0}/supervisor"
  candidate_artifacts = {
    eif = {
      key    = local.candidate_eif_key
      source = abspath(local.eif_source)
      hash   = data.local_file.eif.content_md5
    }
    supervisor = {
      key    = local.candidate_supervisor_key
      source = abspath(local.supervisor_source)
      hash   = data.local_file.supervisor.content_md5
    }
  }
}
...
# Candidate objects are intentionally outside Tofu's S3 object lifecycle. A
# replacement uploads the new PCR-addressed object but never deletes the prior
# candidate; force_destroy on the bucket handles final deployment teardown.
removed {
  from = aws_s3_object.enclave_eif
}

removed {
  from = aws_s3_object.supervisor_binary
}

resource "terraform_data" "candidate_upload" {
  for_each = local.candidate_artifacts
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      if aws s3api head-object --bucket "$BUCKET" --key "$KEY" --region "$AWS_REGION" > /dev/null 2> "$WORK/head.err"; then
        aws s3 cp "s3://$BUCKET/$KEY" "$WORK/existing" --region "$AWS_REGION"
        if ! cmp -s "$SOURCE" "$WORK/existing"; then
          echo "refusing to overwrite retained candidate s3://$BUCKET/$KEY" >&2
          exit 1
        fi
        exit 0
      fi
      if ! grep -Eq '\(404\)|Not Found|NoSuchKey' "$WORK/head.err"; then
        cat "$WORK/head.err" >&2
        exit 1
      fi
      aws s3 cp "$SOURCE" "s3://$BUCKET/$KEY" --region "$AWS_REGION"
    EOT
    environment = {
...
resource "aws_instance" "nitro" {
  ...
  # Wait for IAM and candidate uploads before booting; user_data downloads both
  # candidate objects immediately.
  depends_on = [
    aws_iam_role_policy.enclave,
    terraform_data.candidate_upload,
  ]
  ...
  user_data = templatefile("${path.module}/templates/user_data.sh.tftpl", {
    ...
    app_name                 = var.app_name
    eif_s3_url               = "s3://${aws_s3_bucket.assets.id}/${local.candidate_eif_key}"
    supervisor_binary_s3_url = "s3://${aws_s3_bucket.assets.id}/${local.candidate_supervisor_key}"
    is_kms_key_locked        = var.is_kms_key_locked
  })
...
output "candidate_eif_key" {
  description = "PCR0-addressed S3 key of the candidate EIF."
  value       = local.candidate_eif_key
}

output "candidate_supervisor_key" {
  description = "PCR0-addressed S3 key of the candidate supervisor."
  value       = local.candidate_supervisor_key
}
