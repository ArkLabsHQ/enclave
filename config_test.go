...
func TestValidateSDK(t *testing.T) {
	valid := &Config{SDK: SDKConfig{Rev: "abc", Hash: "sha256-x", VendorHash: "sha256-y"}}
	if err := valid.validateSDK(); err != nil {
		t.Errorf("valid SDK: %v", err)
	}

	for _, field := range []string{"rev", "hash", "vendor_hash"} {
		t.Run("missing_"+field, func(t *testing.T) {
			sdk := SDKConfig{Rev: "abc", Hash: "sha256-x", VendorHash: "sha256-y"}
			switch field {
			case "rev":
				sdk.Rev = ""
			case "hash":
